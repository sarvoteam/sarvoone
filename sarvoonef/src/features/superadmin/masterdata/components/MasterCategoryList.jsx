...
export default function MasterCategoryList({ categories, onEdit, onDelete }) {
  return (
    <div style={{ backgroundColor: '#fff', borderRadius: '12px', border: '1px solid #e5e7eb', overflow: 'hidden' }}>
      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', textAlign: 'left' }}>
        <thead>
          <tr style={{ borderBottom: '2px solid #f3f4f6', backgroundColor: '#fafafa' }}>
            <th style={{ padding: '14px 16px', color: '#4b5563', fontWeight: 600 }}>Category Tag</th>
            <th style={{ padding: '14px 16px', color: '#4b5563', fontWeight: 600 }}>Scope Module</th>
            <th style={{ padding: '14px 16px', color: '#4b5563', fontWeight: 600 }}>Global Code ID</th>
        ...
        <tbody>
          {categories.map(cat => (
            <tr key={cat.id} style={{ borderBottom: '1px solid #f3f4f6' }}>
              ...
              <td style={{ padding: '14px 16px', color: '#4b5563' }}>{cat.module}</td>
              <td style={{ padding: '14px 16px', fontFamily: 'monospace', color: '#6b7280' }}>{cat.code}</td>
